...
const EMAIL_FROM_RAW = String(process.env.EMAIL_FROM || 'Sistema Seguimiento <no-reply@quito-turismo.gob.ec>').trim();
...
const DEFAULT_FROM_EMAIL = (EMAIL_FROM_MATCH?.[2] || EMAIL_FROM_RAW).trim() || 'no-reply@quito-turismo.gob.ec';
...
function buildDefaults() {
  ...
  const defaults = {
    ...
    serverType: normalizeServerType(process.env.SMTP_SERVER_TYPE || 'office365', 'smtp'),
    smtpHost: String(process.env.SMTP_HOST || '').trim(),
    smtpPort: parseInteger(process.env.SMTP_PORT, 587, 1),
    smtpSecure: parseBoolean(process.env.SMTP_SECURE, false),
    requireAuth: parseBoolean(process.env.SMTP_REQUIRE_TLS, true),
    smtpUser: String(process.env.SMTP_USER || '').trim(),
    smtpPassword: smtpPass,
    supervisorEmails: normalizeEmailList(process.env.SUPERVISOR_EMAILS || ''),
    sendTime: String(process.env.SMTP_SEND_TIME || '08:00').trim(),
    timezone: String(process.env.TIMEZONE || 'America/Guayaquil').trim(),
    notifyDelayedStages: parseBoolean(process.env.NOTIFY_DELAYED_STAGES, true),
    delayedStageDays: parseInteger(process.env.DELAYED_STAGE_DAYS, 2, 1),
  ...
  if (process.env.DEBUG_NOTIFICATIONS === 'true') {
    console.log('📧 SMTP Defaults:', {
      host: defaults.smtpHost,
      port: defaults.smtpPort,
      user: defaults.smtpUser,
      hasPassword: Boolean(defaults.smtpPassword),
      requireAuth: defaults.requireAuth,
      secure: defaults.smtpSecure
    });
  }
...
function toConfig(row = null, { includeSecret = false } = {}) {
  ...
  const config = {
    ...
    fromEmail: (row.remitente_email || defaults.fromEmail).trim() || defaults.fromEmail,
    serverType: normalizeServerType(row.tipo_servidor || defaults.serverType, defaults.serverType),
    smtpHost: normalizeText(row.smtp_host, defaults.smtpHost),
    smtpPort: parseInteger(row.smtp_port, defaults.smtpPort, 1),
    smtpSecure: Boolean(row.smtp_secure) || defaults.smtpSecure,
    requireAuth: parseBoolean(row.requiere_auth !== undefined ? row.requiere_auth : defaults.requireAuth, defaults.requireAuth),
    smtpUser: normalizeText(row.smtp_user, defaults.smtpUser),
    smtpPassword: includeSecret ? String(row.smtp_password || defaults.smtpPassword) : '',
    smtpPasswordConfigured: Boolean(row.smtp_password || defaults.smtpPassword),
...
export async function getConfiguracionCorreo({ includeSecret = false } = {}) {
  ...
  const result = toConfig(rows[0] || null, { includeSecret });
  console.log('📧 getConfiguracionCorreo result:', {
    fromEmail: result.fromEmail,
    fromName: result.fromName,
    smtpUser: result.smtpUser,
    hasPassword: Boolean(result.smtpPassword)
  });
  return result;
}
...
export async function obtenerPlantillaHTML() {
  const config = await getConfiguracionCorreo();
  return config.htmlTemplate || DEFAULT_NOTIFICATION_TEMPLATE;
}

export async function guardarPlantillaHTML(plantilla = '') {
  const current = await getConfiguracionCorreo({ includeSecret: true });
  const updated = {
    ...current,
    htmlTemplate: String(plantilla || DEFAULT_NOTIFICATION_TEMPLATE)
  };

  return guardarConfiguracionCorreo(updated);
}
