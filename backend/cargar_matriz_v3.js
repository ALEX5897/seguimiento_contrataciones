...
console.log('📊 CARGANDO TODOS LOS PROCESOS DEL EXCEL (v3)\n');
...
function generarCodigoUnico(codigoOriginal, fila, subtarea, direccion) {
  if (codigoOriginal && codigoOriginal !== 'N/A' && codigoOriginal.trim() !== '') {
    return codigoOriginal;
  }

  // Generar código basado en hash de subtarea + dirección + fila
  const input = `${subtarea}-${direccion}-${fila}`.substring(0, 100);
  const hash = crypto.createHash('md5').update(input).digest('hex').substring(0, 10);
  return `AUTO_${fila}_${hash}`.toUpperCase();
}
...
try {
  ...
  for (const [nombreDireccion, procesos] of Object.entries(procesosPorDireccion)) {
    ...
    let cargados = 0;
    for (let idx = 0; idx < procesos.length; idx++) {
      const proceso = procesos[idx];

      // Generar código único si es necesario
      const codigoFinal = generarCodigoUnico(
      ...
      if (codigoFinal !== proceso.codigo_olympo) {
        codigosGenerados++;
      }
  ...
  console.log(`🔧 Códigos generados (N/A): ${codigosGenerados}\n`);
  ...
  console.log('\n✨ TODOS LOS DATOS CARGADOS Y LISTOS PARA USAR');
