...
// GET /api/configuracion/:clave - Obtener una configuración específica (sin autenticación)
router.get('/:clave', async (req, res) => {
  try {
    const clave = req.params.clave;
    console.log(`📖 Obteniendo configuración: ${clave}`);

    const config = await mysql.getConfiguracion(clave);

    if (!config) {
      console.warn(`⚠️ Configuración no encontrada: ${clave}, retornando valor por defecto`);
      // Retornar valor por defecto para configuraciones conocidas
      if (clave === 'editar_fecha_planificada_direcciones') {
        return res.json({ clave, valor: '0', mensaje: 'Configuración por defecto' });
      }
      return res.status(404).json({ error: `Configuración "${clave}" no encontrada` });
    }
