...
notificacionesRouter.get('/plantilla/html', async (req, res) => {
  try {
    const { obtenerPlantillaHTML } = await import('../services/notificacionesConfig.js');
    const plantilla = await obtenerPlantillaHTML();
    res.json({ plantilla });
  } catch (error) {
    console.error('Error en GET /api/notificaciones/plantilla/html:', error);
    res.status(500).json({ error: error.message || 'No se pudo obtener la plantilla' });
  }
});

// Guardar plantilla HTML del correo
notificacionesRouter.put('/plantilla/html', async (req, res) => {
  try {
    const { guardarPlantillaHTML } = await import('../services/notificacionesConfig.js');
    const { plantilla } = req.body;
    if (!plantilla) {
      return res.status(400).json({ error: 'La plantilla es requerida' });
    }
    const saved = await guardarPlantillaHTML(plantilla);
    res.json({ plantilla: saved });
  } catch (error) {
    console.error('Error en PUT /api/notificaciones/plantilla/html:', error);
    res.status(500).json({ error: error.message || 'No se pudo guardar la plantilla' });
  }
});
