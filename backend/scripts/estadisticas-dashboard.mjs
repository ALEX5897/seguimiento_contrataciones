...
async function obtenerEstadisticas() {
  ...
  try {
    ...
    const procesosActivos = await query(`
      ...
      WHERE
        s.activo = 1
        AND (s.presupuesto_2026_inicial > 0 OR s.presupuesto > 0)
        AND s.activo != 2
    `);
    console.log('\n1. TOTAL DE PROCESOS ACTIVOS VÁLIDOS');
    console.log(`   (presupuesto > 0, no desiertos, estado activo)`);
    ...
    const procesosPAC = await query(`
      ...
      WHERE
        ...
        AND (s.presupuesto_2026_inicial > 0 OR s.presupuesto > 0)
        AND s.activo != 2
        AND (s.pac_no_pac = 'PAC' OR s.tipoPlan = 'PAC')
    `);
    ...
    const procesosNOPAC = await query(`
      ...
      WHERE
        ...
        AND (s.presupuesto_2026_inicial > 0 OR s.presupuesto > 0)
        AND s.activo != 2
        AND (s.pac_no_pac = 'NO PAC' OR s.tipoPlan = 'NO PAC')
    `);
    ...
    const etapasCompletadas = await query(`
      ...
      FROM seguimiento_etapas se
      WHERE
        se.estado = 'completado'
        AND se.subtarea_id IN (
          SELECT id FROM subtareas s
          WHERE
            s.activo = 1
            AND (s.presupuesto_2026_inicial > 0 OR s.presupuesto > 0)
    ...
    const etapasAtrasadas = await query(`
      ...
      FROM seguimiento_etapas se
      WHERE
        se.estado IN ('pendiente', 'en_curso', 'en_revision', 'bloqueada')
    ...
    const etapasEnProgreso = await query(`
      ...
      WHERE
        se.estado IN ('pendiente', 'en_curso', 'en_revision', 'bloqueada')
        AND (se.fecha_planificada >= CURDATE() OR se.fecha_tentativa >= CURDATE() OR (se.fecha_planificada IS NULL AND se.fecha_tentativa IS NULL))
        AND se.fecha_real IS NULL
        AND se.subtarea_id IN (
          SELECT id FROM subtareas s
          WHERE
    ...
    // 9. Porcentajes
    const totalEtapasVal = totalEtapas[0].total || 1;
    const pctCumplimiento = Math.round((etapasCompletadas[0].total / totalEtapasVal) * 100);
    ...
    console.log(`   → Total: ${pctCumplimiento}% (${etapasCompletadas[0].total}/${totalEtapasVal})`);
    console.log(`   → PAC: ${pctPAC}% (${etapasCompletadasPAC[0].total}/${etapasPAC[0].total})`);
    console.log(`   → NO PAC: ${pctNOPAC}% (${etapasCompletadasNOPAC[0].total}/${etapasNOPAC[0].total})`);

    // 10. Porcentajes de etapas por estado
    const pctAtrasadas = Math.round((etapasAtrasadas[0].total / totalEtapasVal) * 100);
    const pctEnProgreso = Math.round((etapasEnProgreso[0].total / totalEtapasVal) * 100);

    console.log('\n10. DISTRIBUCIÓN DE ETAPAS POR ESTADO');
    console.log(`   → Completadas: ${pctCumplimiento}% (${etapasCompletadas[0].total}/${totalEtapasVal})`);
    console.log(`   → Atrasadas: ${pctAtrasadas}% (${etapasAtrasadas[0].total}/${totalEtapasVal})`);
    console.log(`   → En Progreso: ${pctEnProgreso}% (${etapasEnProgreso[0].total}/${totalEtapasVal})`);
