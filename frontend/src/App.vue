...
/////////////LOGO/////////////
const logoQt = 'https://turismo.quito.gob.ec/wp-content/uploads/2024/06/logoQT-1024x166.png'
...
const menuConfig: MenuItem[] = [
  ...
  { key: 'informes', to: '/informes', label: 'Informes', icon: 'ri-file-pdf-line' },
  ...
  { key: 'admin_actividades', to: '/admin/carga-masiva', label: 'Carga Masiva', icon: 'ri-upload-cloud-2-line', isAdmin: true },
...
function connectRealtime() {
  ...
  if (realtimeFailureCount >= MAX_REALTIME_RETRIES) {
    console.warn(`⚠️  Realtime: demasiados intentos fallidos (${realtimeFailureCount}/${MAX_REALTIME_RETRIES}). Abortando reconexión.`)
    return
  }
  ...
  realtimeSource.addEventListener('data-change', () => {
    realtimeFailureCount = 0 // Reset contador en caso de éxito
    scheduleSoftReload()
  })

  realtimeSource.addEventListener('connected', () => {
    realtimeFailureCount = 0 // Reset en conexión exitosa
  ...
  realtimeSource.onerror = () => {
    ...
    const delayMs = Math.min(5000 * realtimeFailureCount, 30000) // Max 30s
    if (realtimeReconnectTimer === undefined && realtimeFailureCount < MAX_REALTIME_RETRIES) {
      console.warn(`⚠️  Realtime error (intento ${realtimeFailureCount}/${MAX_REALTIME_RETRIES}). Reintentando en ${delayMs}ms...`)
